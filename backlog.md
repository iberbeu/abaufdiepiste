...
| ~~FEAT-22~~ | ~~Punkte / Abschlusswertung~~ | ~~Resolved~~ — Interactive per-player wizard in Punkte tab (triggered by "Schlusswertung starten →" button after game end). Flow per player: (1) Talstation reached? Ja→summary, Nein→(2) kreuzungen slope selector (all 4 colours, halved penalty preview) + Beförderungsmittel +/- + zusätzliche Talstationen +/- → (3) summary with penalty breakdown + coin bonus (+5 per remaining joker/gratis coin) + net delta + confirm. Points written to player; coins zeroed. Per-player `schlussgewertungDone` flag persists progress across page reloads. | Rules | — |
