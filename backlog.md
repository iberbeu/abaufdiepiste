...
| FEAT-23 | Punkte | **Details per round.** The table PUNKTVERLAUF shows all rounds with all points that were added each round. This is OK but the user can still not see what concretey happened in that round. By now he should go to the ZUGVERLAUF area to check this, but that is very cumbersome. The solution is that if a player clicks on one of the cells of the table PUNKTVERLAUF, a popup comes with the points obtained that round and how they wre obtained. Example: 2x rote Piste = 8 /n 1x blaue Piste = 2 /n Sehenswürdigkeit = 5 | - | - |
